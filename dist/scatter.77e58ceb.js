...
var svg = d3.select('#scatter').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
...
var yPositionScale = d3.scaleLinear().domain([0, 300]).range([height, 0]);
d3.csv(require('./data/era_classification_completed.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  }).entries(datapoints); // console.log('nested classification look like', nested_type)

  var eras = nested.map(function (d) {
    return +d.key;
  });
  xPositionScale.domain(eras); // // console.log(eras)
  // console.log('nested data looks like', nested)

  svg.selectAll('.circle').data(nested).enter().append('g').attr('class', 'relic-dot').each(function (d) {
    var g = d3.select(this); // console.log('Group is', d)

    var datapoints = d.values;
    console.log(datapoints);
    g.selectAll('.relic-dot').data(datapoints).enter().append('rect').attr('width', 6).attr('height', 6).attr('x', function (d) {
    ...
    legend.selectAll('.legend-entry').data(nested_type).enter().append('g').attr('transform', function (d, i) {
      ...
      var g = d3.select(this); // let datapoints = d.values
      // console.log(datapoints[0])

      g.append('rect').attr('x', 10).attr('y', 0).attr('width', 10).attr('height', 10).attr('fill', colorScale(d.key));
...
},{"d3":"../node_modules/d3/index.js","./data/era_classification_completed.csv":"data/era_classification_completed.csv"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","scatter.js"], null)
//# sourceMappingURL=/scatter.77e58ceb.map