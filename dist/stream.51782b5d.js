...
var svg = d3.select('#stream').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // Normal scales

var xPositionScale = d3.scaleLinear().domain([-1900, 2018]).range([0, width]); // const colorScale = d3.scaleOrdinal(d3.schemeCategory10)

var colorScale = d3.scaleOrdinal().range(['#ffffb2', '#fed976', '#feb24c', '#fd8d3c', '#f03b20', '#983530']);
...
Promise.all([d3.csv(require('./data/era_classification_pct_completed.csv')), d3.csv(require('./data/architecture_completed.csv'))]).then(ready).catch(function (err) {
...
function ready(_ref) {
  ...
  var wide = d3.nest().key(function (d) {
    return d.era_year;
  }).rollup(function (rows) {
    var d = {};
    d.era_year = rows[0].era_year;
    rows.forEach(function (row) {
      return d[row.classification_en] = row.number_pct;
    });
    return d;
  }).entries(datapoints).map(function (d) {
    return d.value;
  });
  ...
  var nested_type = d3.nest().key(function (d) {
    return d.classification_en;
  }).entries(datapoints);
  console.log('nested classification look like', nested_type); // TODO add an offset

  var keys = Object.keys(wide[0]).filter(function (col) {
    return col !== 'era_year';
  });
  console.log(keys);
  var stack = d3.stack().keys(keys).order(d3.stackOrderDescending).offset(d3.stackOffsetSilhouette); // TODO stack them

  var layers = stack(wide); // TODO change the domain for the streamgraph

  yPositionScale.domain([d3.min(layers, function (layer) {
  ...
  svg.selectAll('path').data(layers).enter().append('path').attr('class', 'relics').attr('fill', function (d) {
    ...
    div.html(d.key).style('left', d3.event.pageX + 'px').style('top', d3.event.pageY - 28 + 'px').style('display', 'block');
  }).on('mouseover', function (d, i) {
    div.transition().duration(200).style('opacity', 1);
    div.html(d.key).style('left', d3.event.pageX + 'px').style('top', d3.event.pageY - 28 + 'px');
    d3.select('#relic' + i).transition().attr('opacity', 1);
    d3.selectAll('.relics').attr('opacity', '0.2'); // d3.selectAll('.relics').attr('opacity', '0.5')
    // d3.select(this).attr('opacity', 1)
  }).on('mouseout', function (d, i) {
    div.transition().duration(200).style('opacity', 0);
    d3.selectAll('.relics').attr('opacity', '1').attr('fill', function (d) {
      return colorScale(d.key);
    });
  }); //stack for dotplot

  var nested_year = d3.nest().key(function (d) {
    return +d.era_year;
  }).entries(datapoints_all);
  console.log('Datapoints_all nested year look like', nested_year);
  var nestedYear = nested_year.map(function (d) {
    return +d.key;
  });
  console.log('nestedYear look like', nestedYear);
  var stack_dots = d3.stack().keys(keys).order(d3.stackOrderDescending); // svg
  //    .selectAll('.circles')
  //    .data(datapoints_all)
  //    .enter()
  //    .append('circle')
  ...
  svg.selectAll('.circles').data(nested_year).enter().append('circle').attr('r', 3).attr('cx', function (d) {
  ...
  .attr('cy', function (d) {
    ...
    console.log(i);
    return height / 2 - i * 3;
  }).attr('fill', 'red'); //    .style('visibility', 'hidden')

  svg.append('circle').attr('class', 'image').attr('r', 60).attr('cx', xPositionScale(700)).attr('cy', 60).attr('fill', 'red').style('visibility', 'hidden'); // Add in your axes
  // var tickLabels = [
  //     'Xia dynasty',
  //     'Shang dynasty',
  //     'Zhou dynasty',
  ...
  var legend = svg.append('g').attr('transform', 'translate(10, 0)');
  legend.selectAll('.legend-entry').data(nested_type).enter().append('g').attr('transform', function (d, i) {
    return "translate(0,".concat(i * 25, ")");
  }).attr('class', 'legend-entry').each(function (d) {
    var g = d3.select(this);
    g.append('rect').attr('x', 10).attr('y', 0).attr('width', 6).attr('height', 6).attr('fill', colorScale(d.key));
    g.append('text').text(d.key).attr('x', 15).attr('y', 5).attr('dx', 17).attr('font-size', 10).attr('fill', 'black').attr('alignment-baseline', 'middle');
  }); // year reference

  svg.append('text').text('Year in CE').attr('x', xPositionScale(0)).attr('y', height + 20).attr('font-size', 11).attr('fill', 'black').attr('font-weight', 500).attr('text-anchor', 'start').attr('dx', '5px');
  svg.append('text').text('Year in BC').attr('x', xPositionScale(0)).attr('y', height + 20).attr('font-size', 11).attr('fill', 'black').attr('font-weight', 500).attr('text-anchor', 'end').attr('dx', '-5px'); //* axes part *//

  var years2 = d3.nest().key(function (d) {
    return d;
  }).entries(years).map(function (d) {
    return d.key;
  });
  var xAxis = d3.axisBottom(xPositionScale).tickValues(lines).tickSize(-height).tickFormat(d3.format("d")); //  .tickFormat(function(d,i){ 
  //   console.log('d is', d, 'i is', i)
  //   return tickLabels[i]
  // })

  svg.append('g').attr('class', 'axis x-axis').attr('transform', 'translate(0,' + height + ')').call(xAxis);
  svg.selectAll('x-axis').style("text-anchor", "middle");
  svg.selectAll('.x-axis line').attr('stroke-dasharray', '3 3').attr('stroke-linecap', 'round').attr('stroke', '#e6e6e6');
  svg.select('.axis').lower();
  var yAxis = d3.axisLeft(yPositionScale).tickFormat(function (d) {
    console.log(d);
  ...
  svg.selectAll('.domain').remove(); // SCROOLLING  PART //

  d3.select('#all-line').on('stepin', function () {
    svg.selectAll('.forbidden-city').style('visibility', 'hidden').transition().attr('cy', 0);
    svg.selectAll('.firstbatch').style('visibility', 'hidden').transition().attr('cy', 0);
    svg.selectAll('.relics').style('pointer-events', 'auto');
    svg.selectAll('.image').style('visibility', 'hidden');
  });
  d3.select('#first-batch').on('stepin', function () {
    console.log('I am step into first batch');
    svg.selectAll('.relics').style('pointer-events', 'none');
    svg.selectAll('.firstbatch').style('visibility', 'visible').transition().duration(function (d) {
      return Math.random() * 1000;
    }).attr('opacity', 1).attr('cy', height / 2);
    svg.selectAll('.forbidden-city').style('visibility', 'hidden').transition().attr('cy', 0);
    svg.selectAll('.circles').on('mouseover', function (d) {
      div.transition().style('opacity', 0.9);
      div.html(d.name_en + '<br>' + d.era_en).style('left', d3.event.pageX + 'px').style('top', d3.event.pageY - 28 + 'px');
    }).on('mouseout', function (d, i) {
      div.transition().style('opacity', 0);
    });
  });
  d3.select('#gugong').on('stepin', function () {
    console.log('I am step into gugong'); // svg.selectAll('.circles').transition().style('visibility', 'hidden')

    svg.selectAll('.relics').style('pointer-events', 'none'); // svg.selectAll('.image').style('visibility', 'visible')

    svg.selectAll('.firstbatch').attr('opacity', 0).attr('cy', 0).transition();
    svg.selectAll('.yuetan').attr('opacity', 0).attr('cy', 0).transition();
    svg.selectAll('.forbidden-city').style('visibility', 'visible').raise().transition().attr('opacity', 1).attr('cy', height / 2).attr('fill', 'blue').attr('r', 6);
    svg.selectAll('.circles').on('mouseover', function (d) {
      div.transition().style('opacity', 0.9);
      div.html(d.name_en + '<br>' + d.era_en).style('left', d3.event.pageX + 'px').style('top', d3.event.pageY - 28 + 'px');
    }).on('mouseout', function (d, i) {
  ...
  d3.select('#changcheng').on('stepin', function () {
    console.log('I am step into changcheng');
    svg.selectAll('.relics').style('pointer-events', 'none');
    svg.selectAll('.firstbatch').attr('opacity', 0).attr('cy', height / 2).transition();
    svg.selectAll('.forbidden-city').transition().attr('opacity', 0).attr('cy', 0);
    svg.selectAll('.yuetan').style('visibility', 'visible').raise().transition().attr('opacity', 1).attr('cy', height / 2).attr('fill', 'blue').attr('r', 6);
    svg.selectAll('.circles').on('mouseover', function (d) {
      div.transition().style('opacity', 0.9);
      div.html(d.name_en + '<br>' + d.era_en).style('left', d3.event.pageX + 'px').style('top', d3.event.pageY - 28 + 'px');
    }).on('mouseout', function (d, i) {
...
},{"d3":"../node_modules/d3/index.js","./data/era_classification_pct_completed.csv":"data/era_classification_pct_completed.csv","./data/architecture_completed.csv":"data/architecture_completed.csv"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","stream.js"], null)
//# sourceMappingURL=/stream.51782b5d.map