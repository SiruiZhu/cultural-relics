...
},{"topojson-client":"../node_modules/topojson/node_modules/topojson-client/index.js","topojson-server":"../node_modules/topojson/node_modules/topojson-server/index.js","topojson-simplify":"../node_modules/topojson/node_modules/topojson-simplify/index.js"}],"data/china_shapefile.topojson":[function(require,module,exports) {
module.exports = "/china_shapefile.975e9bf7.topojson";
},{}],"data/province_region.csv":[function(require,module,exports) {
module.exports = "/province_region.0353b583.csv";
...
var svg = d3.select('#map_chart').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.top + margin.bottom).append('g').attr('transform', "translate(".concat(margin.left, ", ").concat(margin.top, ")"));
var projection = d3.geoMercator();
var path = d3.geoPath().projection(projection); // const colorScale = d3
//   .scaleOrdinal()
//   .range([
//         '#ffffb2',
//         '#fed976',
...
var colorScale = d3.scaleOrdinal().range(d3.schemeBrBG);
var radiusScale = d3.scaleSqrt().domain([10, 300]).range([1, 3.5]);
var xPositionScale = d3.scaleLinear().domain([0, 300]).range([0, width]);
var yPositionScale = d3.scaleBand().range([0, height]).padding(0.2);
Promise.all([d3.json(require('./data/china_shapefile.topojson')), d3.csv(require('./data/province_region.csv')), d3.csv(require('./data/architecture_completed.csv'))]).then(ready).catch(function (err) {
...
function ready(_ref) {
  var _ref2 = _slicedToArray(_ref, 3),
      json = _ref2[0],
      datapoints_number = _ref2[1],
      datapoints_points = _ref2[2];

  // console.log(json.objects)
  var nested_region = d3.nest().key(function (d) {
    return d.region;
  }).entries(datapoints_number); // console.log('nested data look like', nested)
  // MAP PART

  var mapGroup = svg.append('g').attr('id', 'map').attr('transform', 'translate(-80, 0)');
  var nested_era = d3.nest().key(function (d) {
    return d.era_replace;
  }).entries(datapoints_points);
  var mainland = topojson.feature(json, json.objects.CHN_adm1);
  var hk = topojson.feature(json, json.objects.HKG_adm1);
  var mc = topojson.feature(json, json.objects.MAC_adm1);
  var taiwan = topojson.feature(json, json.objects.TWN_adm1);
  console.log(mainland.features);
  projection.fitSize([width, height], mainland); // projection.scale(projection.scale() * 0.8)
  ...
  mapGroup.append('g').selectAll('.mainland').data(mainland.features).enter().append('path').attr('class', 'mainland').attr('d', path).attr('fill', function (d) {
    // console.log(d)
  ...
  }).attr('opacity', 0.9).attr('stroke', 'white').attr('stroke-width', 0.1).on('mouseenter', function (d) {
    d3.select(this).attr('stroke', 'white').attr('stroke-width', 2); // console.log(d)
  }).on('mouseleave', function (d) {
    d3.select(this).attr('stroke', 'none');
  });
  mapGroup.append('g').selectAll('.taiwan').data(taiwan.features).enter().append('path').attr('class', 'taiwan').attr('d', path).attr('fill', '#E0E0E0').attr('stroke', 'white').attr('stroke-width', 0.1);
  mapGroup.append('g').selectAll('.mc').data(mc.features).enter().append('path').attr('class', 'mc').attr('d', path).attr('fill', '#E0E0E0');
  mapGroup.append('g').selectAll('.hk').data(hk.features).enter().append('path').attr('class', 'hk').attr('d', path).attr('fill', '#E0E0E0');
  mapGroup.selectAll('.province-label').data(mainland.features).enter().append('text').attr('class', 'province-label').text(function (d) {
  ...
  }).attr('text-anchor', 'middle').attr('alignment-baseline', 'middle').attr('font-size', 10).attr('opacity', 0);
  mapGroup.selectAll('.relics').data(datapoints_points).enter().append('circle').attr('class', 'relics').attr('r', 2).attr('opacity', 0.6).attr('fill', function (d) {
    return colorScale(d.era_replace);
  }).attr('transform', function (d) {
  ...
  var nested_province = d3.nest().key(function (d) {
    return d.province_en;
  ...
  var provinces = nested_province.map(function (d) {
    return d.key;
  });
  console.log('provinces', provinces);
  yPositionScale.domain(provinces);
  var barsGroup = svg.append('g').attr('id', 'bars');
  barsGroup.selectAll('.bar').data(nested_province).enter().append('rect').attr('class', function (d) {
    // give 'coal' a class of 'coal'
    // give 'nuclear power' a class of 'nuclear-power'
    return d.key.toLowerCase().replace(' ', '');
  }).classed('bar', true).attr('x', 0).attr('y', function (d) {
    return yPositionScale(d.key);
  }).attr('width', 0).attr('height', yPositionScale.bandwidth()).attr('fill', 'red');
  d3.select('#draw-bars-1').on('click', function () {
    console.log("I was clicked"); // Move the powerplants to where the bars start

    svg.selectAll('.relics').transition().duration(1000).attr('transform', function (d) {
      var yPosition = yPositionScale(d.province_en) + yPositionScale.bandwidth() / 2; // console.log(d.province_en)

      return "translate(80,".concat(yPosition, ")");
    }).transition().attr('opacity', 0); // Show the bars

    barsGroup.transition().duration(1500).style('opacity', '1'); // Grow the bars

    svg.selectAll('.bar').transition().delay(1000) // delay until the dots are in place
    .duration(1500).attr('width', function (d) {
      return xPositionScale(d.value);
    });
    mapGroup.transition().duration(1500).style('opacity', '0'); // LEGEND

    var labelGroup = svg.append('g').attr('transform', 'translate(200, 200)').attr('id', 'legend');
    labelGroup.selectAll('.label-circle').data(nested_region).enter().append('circle').attr('transform', function (d, i) {
      return "translate(0,".concat(i * 20, ")");
    }).attr('class', 'legend-entry').each(function (d) {
      var g = d3.select(this);
      g.append('circle').attr('r', 5).attr('cx', 0).attr('cy', 0).attr('fill', colorScale(d.key));
      g.append('text').text(d.key).attr('dx', 10).attr('alignment-baseline', 'middle');
      g.append('rect').attr('x', -8).attr('y', -9).attr('width', 400).attr('height', 18).attr('fill', '#fcfcfc').lower();
    });
  });
  d3.selectAll('#draw-bars-2').on('click', function () {
    console.log("I was clicked");
    barsGroup.transition().duration(1500).style('opacity', '0');
    mapGroup.transition().duration(1500).style('opacity', '0.8');
    svg.selectAll('.relics').transition().duration(1000).attr('transform', function (d) {
      var coords = projection([d.longitude, d.latitude]);
      return "translate(".concat(coords, ")");
    }).transition().attr('opacity', 1);
    svg.selectAll('.province-label').transition().duration(200).attr('opacity', 0);
  });
  d3.selectAll('#draw-map').on('click', function () {
    console.log("I was clicked");
    barsGroup.transition().duration(1500).style('opacity', '0');
    mapGroup.transition().duration(1500).style('opacity', '1');
    svg.selectAll('.relics').transition().duration(200).attr('transform', function (d) {
      var coords = projection([d.longitude, d.latitude]);
      return "translate(".concat(coords, ")");
    }).attr('opacity', 0);
    svg.selectAll('.province-label').transition().duration(200).attr('opacity', 1);
  });
  var xAxis = d3.axisBottom(xPositionScale);
  barsGroup.append('g').attr('class', 'axis x-axis').attr('transform', 'translate(0,' + height + ')').call(xAxis);
  var yAxis = d3.axisLeft(yPositionScale);
  barsGroup.append('g').attr('class', 'axis y-axis').call(yAxis);
...
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","./data/china_shapefile.topojson":"data/china_shapefile.topojson","./data/province_region.csv":"data/province_region.csv","./data/architecture_completed.csv":"data/architecture_completed.csv"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
