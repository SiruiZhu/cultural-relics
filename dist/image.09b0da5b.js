...
exports.default = _default;
},{"d3":"../node_modules/d3/index.js"}],"data/Mingarchitecture.csv":[function(require,module,exports) {
module.exports = "/Mingarchitecture.5f4c533d.csv";
...
var svg = d3.select('#image-wall').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
svg.append('rect').attr('width', width).attr('height', height).attr('opacity', 0);

var xPositionScale = _fisheye.default.scale(d3.scaleLinear).range([0, width]).focus(width / 2).distortion(4);

d3.csv(require('./data/Mingarchitecture.csv')).then(ready).catch(function (err) {
  return console.log('Failed on', err);
});

function ready(datapoints) {
  xPositionScale.domain([0, datapoints.length]);
  var holders = svg.selectAll('.image-holder').data(datapoints).enter().append('g').attr('class', 'image-holder').attr('transform', function (d, i) {
    var xPosition = xPositionScale(i);
    return "translate(".concat(xPosition, ", 0)");
  });
  holders.append('image').attr('xlink:href', function (d) {
    return d.image_url;
  }).attr('class', function (d) {
    // console.log(d['name_en'].replace(/\s+/g, '-').toLowerCase())
    return d['name_en'].replace(/\s+/g, '-').toLowerCase();
  }).classed('name-text', true).attr('height', height);
  holders.append('rect').attr('stroke', 'black').attr('height', height).attr('width', 200).attr('fill', 'none');
  ...
  function redraw() {
    var _d3$mouse = d3.mouse(this),
        ...
        mouseX = _d3$mouse2[0],
        mouseY = _d3$mouse2[1]; // focus the x axis where the mouse is


    xPositionScale.focus(mouseX);
    svg.selectAll('.image-holder').attr('transform', function (d, i) {
      var xPosition = xPositionScale(i);
      return "translate(".concat(xPosition, ", 0)");
    });
  }

  var drag = d3.drag().on('start', redraw).on('drag', redraw);
  svg.on('mousemove', redraw).on('click', redraw).call(drag);
} // play with distortion and height so images look the righ size
},{"d3":"../node_modules/d3/index.js","./fisheye":"fisheye.js","d3-scale":"../node_modules/d3-scale/src/index.js","./data/Mingarchitecture.csv":"data/Mingarchitecture.csv"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","image.js"], null)
//# sourceMappingURL=/image.09b0da5b.map